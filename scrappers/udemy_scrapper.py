...
class UdemyScrapper(BaseScrapper):
    ...
    def find_course_details(self, udemy_id):
        ...
        if "price_text" in landing_data:
            price_details = self.find_course_price_details(
                landing_data["price_text"].get("data")
            )
        ...
        if "instructor_bio" in landing_data:
            instructors = self.find_course_instructors(
                landing_data["instructor_bio"].get("data")
            )
        if "slider_menu" in landing_data:
            headline_data = self.find_course_headline_data(
                landing_data["slider_menu"].get("data")
            )
        if "curriculum" in landing_data:
            curriculum = self.find_course_curriculum(
                landing_data["curriculum"].get("data")
            )
        if "discount_expiration" in landing_data:
            discount_expiration = self.find_course_discount_expiration(
                landing_data["discount_expiration"].get("data")
            )
        if "reviews" in landing_data:
            ratings = self.find_course_ratings(
                landing_data["reviews"].get("display_app_args")
            )
    ...
    def find_course_price_details(self, data):
        ...
        if pricing_result:
            ...
            if list_price:
                return {
                    "discount_code": pricing_result.get("code"),
                    "price": 0,  # NOTE: hardcoded pricing_result.get("price", {}).get("amount", 0),
                    "old_price": list_price.get("amount", 0),
                    "currency": pricing_result.get("price", {}).get(
                        "currency_symbol", "$"
                    ),
                    "discount_percent": pricing_result.get(
    ...
    def find_course_incentives(self, data):
        if data is None:
            return {}
        fields = {
            "video_content_length": ("video_content_length", 0),
    ...
    def find_course_instructors(self, data):
        ...
        for instructor in instructors_data:
            instructor_details = {
                "full_name": instructor.get("display_name"),
                "job_title": instructor.get("job_title"),
                "udemy_id": instructor.get("id"),
                "description": instructor.get("description")
                if len(instructor.get("description"))
                else None,
    ...
    def find_course_basic_info(self, data):
        if data is None:
            return {}
        primary_category = data.get("primary_category", {}).get(
            "title"
        )  # TODO: maybe to add title cleaned
            ...
            lectures.append(
                {
                    ...
                    "content_length": convert_duration_from_str_to_int(
                        section.get("content_length_text")
                    ),
                }
            )
            item_counter = 1
                ...
                lectures.append(
                    {
                        ...
                        "description": item.get("description")[
                            :CURRICULUM_ITEM_DESCRIPTION_LEN
                        ],
